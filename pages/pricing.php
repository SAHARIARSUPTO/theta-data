...
<div class="container py-5">
    ...
    <ul class="nav nav-tabs justify-content-center mb-4" id="dataTypeTabs" role="tablist">
        <?php $first = true; foreach (array_keys($plans) as $dataType): ?>
            <li class="nav-item" role="presentation">
                <button class="nav-link <?= $first ? 'active' : '' ?>" id="<?= strtolower($dataType) ?>-tab" data-bs-toggle="tab" data-bs-target="#<?= strtolower($dataType) ?>" type="button" role="tab" aria-controls="<?= strtolower($dataType) ?>" aria-selected="<?= $first ? 'true' : 'false' ?>">
                    <?= $dataType ?>
                </button>
            </li>
    ...
    <div class="tab-content" id="dataTypeTabsContent">
        <?php $first = true; foreach ($plans as $dataType => $modes): ?>
            <div class="tab-pane fade <?= $first ? 'show active' : '' ?>" id="<?= strtolower($dataType) ?>" role="tabpanel" aria-labelledby="<?= strtolower($dataType) ?>-tab">
    ...
    <div id="subscription-form-section" class="row justify-content-center" style="display: none;">
        <div class="col-lg-8">
            <div class="subscription-form">
                ...
                <form method="POST" action="pricing_page.php">
...
<script>
    ...
    function showForm(dataType, planName) {
        ...
        const priceElement = document.querySelector(`#${activeTabId} .price-mode-${currentPaymentMode}.active[data-plan="${planName}"]`);
